...
class MethodologyDesignerSkill:
    ...
    def run_action(self, action_id, inputs, host):
        action_map = {
            ...
            'suggest_analysis': self._run_suggest_analysis,
        }
...
"""
    ...
    def _run_suggest_analysis(self, inputs, host):
        data_type = inputs.get('data_type', 'quantitative')
        research_question = inputs.get('research_question', '').strip()
        sample_size = inputs.get('sample_size', '').strip()
        ...
        prompt = f"""## 任务
根据数据类型和研究问题推荐合适的分析方法。

## 研究问题
{research_question}

## 数据类型
{data_type_map.get(data_type, data_type)}

## 样本量
{sample_size if sample_size else '未指定'}
...
"""
        ...
        result = host.call_llm(
            prompt,
            system='你是一名数据分析专家，擅长根据研究问题和数据类型推荐合适的分析方法。',
        )

        return {
            'action_id': 'suggest_analysis',
            'data_type': data_type,
            'research_question': research_question,
